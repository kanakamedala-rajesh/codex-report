...
export interface Statement { run(...params: Value[]): unknown; all(...params: Value[]): Record<string, unknown>[]; get(...params: Value[]): Record<string, unknown> | undefined }
export interface Database { exec(sql: string): unknown; prepare(sql: string): Statement; close(): void }
export function openDatabase(file: string, readonly = false): { db: Database; backend: string } {
  ...
  if (process.env.CODEX_REPORT_SQLITE_BACKEND !== 'builtin') {
    try {
      const Constructor = req('libsql') as new (p: string, o: object) => Database;
      return { db: new Constructor(file, { readonly, fileMustExist: readonly }), backend: 'libsql 0.5.29' };
    } catch (error) {
      if (process.env.CODEX_REPORT_SQLITE_BACKEND === 'libsql') throw error;
    }
  }
  try {
    const native = req('node:sqlite') as { DatabaseSync: new (p: string, o: object) => Database };
    return { db: new native.DatabaseSync(file, { readOnly: readonly }), backend: 'node:sqlite' };
  } catch {
    throw new Error('No SQLite backend available. Install the optional libsql dependency for Node 18/20, or use a Node release with node:sqlite. No compiler fallback is invoked.');
  }
}
export class Lease {
  private file: string; private nonce = crypto.randomUUID();
  constructor(home: string) {
    this.file = path.join(home, 'writer.lock');
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        fs.writeFileSync(this.file, JSON.stringify({ pid: process.pid, nonce: this.nonce }), { flag: 'wx', mode: 0o600 }); return;
      } catch (err) {
        if ((err as NodeJS.ErrnoException).code !== 'EEXIST') throw err;
        let pid: number;
        try { pid = Number((JSON.parse(fs.readFileSync(this.file, 'utf8')) as { pid: unknown }).pid); } catch { throw new Error('Writer lock is unreadable; inspect it with all collectors stopped.'); }
        if (!Number.isInteger(pid) || pid <= 0) throw new Error('Invalid writer lock; manual inspection required.');
        try { process.kill(pid, 0); throw new Error('Collector is running. Use its dashboard or stop it before maintenance.'); }
  ...
  close(): void {
    try { const value = JSON.parse(fs.readFileSync(this.file, 'utf8')) as { nonce: unknown };
      if (value.nonce === this.nonce) fs.unlinkSync(this.file); } catch {}
  }
}
const SCHEMA = `
...
`;
export class Store {
  readonly db: Database; readonly backend: string; private lease: Lease | null = null;
  constructor(readonly home: string, readonly readOnly = false) {
    if (!readOnly) { privateDir(home); this.lease = new Lease(home); }
    try {
      const opened = openDatabase(path.join(home, 'usage.sqlite3'), readOnly); this.db = opened.db; this.backend = opened.backend;
  ...
  revision(): number { return Number(this.db.prepare("SELECT value FROM meta WHERE key='revision'").get()?.value ?? 0); }
  bump(): void { this.db.prepare("UPDATE meta SET value=CAST(value AS INTEGER)+1 WHERE key='revision'").run(); }
  writeBatch(batch: Batch): number {
    ...
    for (const s of batch.samples) {
      ...
      this.db.prepare('INSERT INTO samples VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(s.id,s.thread,s.turn,s.rootThread,s.rootTurn,s.at,s.model,s.effort,s.tier,s.account,s.device,s.format,s.input,s.cached,s.write,s.output,s.reasoning,s.pricePico,s.priceSnapshot,s.priceReason);
      added++;
    }
    for (const q of batch.quotas) this.db.prepare('INSERT OR IGNORE INTO quotas VALUES(?,?,?,?,?,?,?,?,?,?)').run(q.id,q.thread,q.at,q.account,q.limit,q.window,q.minutes,q.used,q.resets,q.reached);
    for (const t of batch.tools) this.db.prepare('INSERT OR IGNORE INTO tools VALUES(?,?,?,?,?)').run(t.id,t.thread,t.turn,t.kind,t.at);
  ...
  putTurn(t: Turn): void {
    const old = this.db.prepare('SELECT * FROM turns WHERE thread=? AND id=?').get(t.thread,t.id) as unknown as Turn | undefined;
    if (old) {
      const rank: Record<Turn['status'],number> = { unknown:0, running:1, stopping:2, interrupted:3, completed:3, failed:3 };
      const useNew = rank[t.status] >= rank[old.status] && (t.status !== 'unknown') && (!old.ended || !t.ended || t.ended >= old.ended);
      t = { ...old, model: t.model || old.model, effort: t.effort || old.effort, tier: t.tier || old.tier,
        rootThread: t.rootThread || old.rootThread, rootTurn: t.rootTurn || old.rootTurn,
        ...
        status: useNew ? t.status : old.status, error: useNew ? t.error ?? old.error : old.error };
      this.db.prepare('UPDATE turns SET rootThread=?,rootTurn=?,started=?,ended=?,durationMs=?,status=?,error=?,model=?,effort=?,tier=? WHERE thread=? AND id=?').run(t.rootThread,t.rootTurn,t.started,t.ended,t.durationMs,t.status,t.error,t.model,t.effort,t.tier,t.thread,t.id);
    } else this.db.prepare('INSERT INTO turns VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').run(t.thread,t.id,t.rootThread,t.rootTurn,t.started,t.ended,t.durationMs,t.status,t.error,t.model,t.effort,t.tier);
  }
  addIssue(scope: string, code: string, at = new Date().toISOString()): void {
    this.db.prepare('INSERT INTO issues VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET at=excluded.at').run(hash(scope + ':' + code),scope,code,at);
  }
  audit(action: string, detail: unknown): void {
    this.db.prepare('INSERT INTO audit VALUES(?,?,?,?)').run(crypto.randomUUID(),new Date().toISOString(),action,JSON.stringify(detail));
  }
  samples(): Sample[] { return this.db.prepare('SELECT * FROM effective_samples ORDER BY at,id').all() as unknown as Sample[]; }
  ...
  close(): void { try { this.db.close(); } finally { this.lease?.close(); this.lease = null; } }
}
