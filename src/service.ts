...
export function runtime(home: string): Runtime | null {
  try {
    const r = object(readJson(path.join(home, 'runtime.json')));
    if (Number.isInteger(r.port) && Number(r.port) > 0 && Number(r.port) < 65536)
      return r as unknown as Runtime;
  } catch {}
  return null;
}
export async function rpc(
  ...
  const r = runtime(home);
  if (!r) throw new Error('Dashboard collector is not running.');
  ...
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on('data', (b: Buffer) => {
          ...
          if (count > 32 * 1024 * 1024) {
            req.destroy(new Error('Response exceeds bound.'));
            return;
          }
          chunks.push(b);
        });
        res.on('end', () => {
          try {
            const o: unknown = JSON.parse(Buffer.concat(chunks).toString('utf8'));
            if (res.statusCode !== 200)
              reject(new Error(String(object(o).error ?? 'Local request failed.')));
            else resolve(o);
          } catch (e) {
            reject(e);
          }
    ...
    req.on('error', reject);
    req.setTimeout(timeout, () => req.destroy(new Error('Local collector timed out.')));
...
export async function startService(home: string, open = false): Promise<Service> {
  ...
  const worker = new Worker(path.join(__dirname, 'worker.js'), { workerData: { home } });
  let sequence = 0;
  ...
  worker.on('exit', (code) => {
    if (code !== 0) rejectAll(new Error('Collector worker stopped unexpectedly.'));
  });
  ...
  const call = (command: string, payload?: unknown): Promise<unknown> =>
    new Promise((resolve, reject) => {
      const id = ++sequence;
      const timer = setTimeout(
        () => {
          pending.delete(id);
          reject(new Error('Collector is busy; retry after reconciliation.'));
        },
        command === 'hook' ? 2000 : 15000,
      );
  ...
  const server = http.createServer((req, res) => {
    void (async () => {
      ...
      const bearer = req.headers.authorization?.startsWith('Bearer ')
        ...
        : '';
      const internal = safeEqual(bearer, config.token);
      const cookie = (req.headers.cookie ?? '')
        .split(';')
        .map((v) => v.trim())
        .find((v) => v.startsWith('codex_report_session='))
        ?.split('=')[1];
      const authorized = internal || Boolean(cookie && sessions.has(cookie));
      if (url.pathname === '/api/session' && req.method === 'POST') {
        const input = object(await body(req));
        if (typeof input.token !== 'string' || !safeEqual(input.token, config.token)) {
          json(res, 403, { error: 'Invalid access token.' });
          return;
        }
        ...
        res.setHeader(
          'Set-Cookie',
          `codex_report_session=${id}; HttpOnly; SameSite=Strict; Path=/`,
        );
      ...
      if (url.pathname.startsWith('/api/')) {
        if (!authorized) {
          json(res, 401, {
            error: 'Open the tokenized dashboard address printed by codex-report start.',
          });
          return;
        }
        if (req.method === 'GET' && url.pathname === '/api/report') {
          json(res, 200, await call('report', Object.fromEntries(url.searchParams)));
          return;
        }
        if (req.method === 'GET' && url.pathname === '/api/status') {
          const state = object(await call('revision'));
          json(res, 200, {
            ...state,
            startedAt: started,
            version: VERSION,
        ...
        if (internal && req.method === 'POST' && url.pathname === '/api/hook') {
          json(res, 200, await call('hook', await body(req)));
          return;
        }
        if (internal && req.method === 'POST' && url.pathname === '/api/sync') {
          json(res, 200, await call('sync'));
          return;
        }
        if (internal && req.method === 'POST' && url.pathname === '/api/stop') {
  ...
  writeJson(path.join(home, 'runtime.json'), {
    pid: process.pid,
    port: localPort,
    startedAt: started,
  });
  ...
  if (open)
    try {
      ...
      process.stderr.write('Browser could not be opened automatically; use the printed address.\n');
    }
  let closing = false;
  shutdown = async () => {
    if (closing) return;
    ...
    await worker.terminate();
    const r = runtime(home);
    if (r?.pid === process.pid) fs.rmSync(path.join(home, 'runtime.json'), { force: true });
  };
  return { url, close: shutdown };
}
export function publicConfig(c: Config): Record<string, unknown> {
  return { ...c, token: '[redacted]' };
}
